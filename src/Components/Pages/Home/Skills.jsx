    ...
    <div>
        ...
        <span className='lg:hidden absolute w-[60%] h-[600px] rounded-full bg-gradient-to-r from-red-500 to-orange-500 bottom-0 animate-pulse blur-[100px]'></span>
            <div className='relative grid grid-cols-2 md:grid-cols-3 lg:grid lg:grid-cols-3 xl:grid-cols-4 justify-evenly  gap-4 lg:gap-10 py-10 lg:px-5'>
                {
                    skillsData.map((item) => (
                        <div key={item.id} className='flex flex-wrap justify-center gap-10 w-auto lg:w-[280px] h-auto lg:h-[160px] rounded-md bg-slate-900 shadow-sm shadow-slate-700 hover:shadow-2xl hover:shadow-blue-800 lg:hover:scale-[1.2] duration-500 py-2'>
                            <div className=' flex-row py-2'>
                                <span className='inline-block px-3 text-center text-7xl lg:text-8xl' style={{
                                    color: item.color
                                }}>{item.icon}</span>
                                <div>
                                <p className=' text-center poppins-bold lg:poppins-bolder text-white'>{item.skill}</p>
                                </div>
                            </div>
                        </div>
                    ))
